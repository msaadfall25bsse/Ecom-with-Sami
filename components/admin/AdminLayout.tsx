...
export function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // 3. Silent Background Verification
  useEffect(() => {
    if (typeof window === 'undefined') return;

    const token = localStorage.getItem('sami_admin_token');

    // If no token and not on login page, show unauthorized prompt
  ...
  // If unauthorized and session expired, render graceful login prompt
