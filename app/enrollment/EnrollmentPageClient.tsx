...
export function EnrollmentPageClient({ initialContent, serverRemainingSeconds }: EnrollmentPageClientProps) {
  ...
  const [screenshotReviews, setScreenshotReviews] = useState(
    initialContent?.screenshot_reviews || defaultCmsContent.screenshot_reviews
  );
  ...
  useEffect(() => {
    const syncData = async () => {
      // 1. Instant hydration from localStorage for 0ms transitions
      try {
        const cachedCheckout = localStorage.getItem('sami_cms_checkout_page');
        if (cachedCheckout) {
          const parsed = JSON.parse(cachedCheckout);
          if (parsed && typeof parsed === 'object') {
            setCheckoutContent(parsed);
          }
        }
        const cachedMethods = localStorage.getItem('sami_cms_payment_methods');
        if (cachedMethods) {
      ...
      // 2. Fetch fresh from public CMS API route with cache buster
      try {
        ...
        if (res.ok) {
          ...
          if (data.success && data.sections) {
            if (data.sections.checkout_page) {
              ...
              try {
                localStorage.setItem('sami_cms_checkout_page', JSON.stringify(data.sections.checkout_page));
              } catch (e) {}
            }
